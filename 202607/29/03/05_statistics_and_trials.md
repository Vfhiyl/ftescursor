...
| 临床向标题过滤 | **35** |
| Crossref 去重（标题关键词过滤） | **57** |
| ClinicalTrials.gov 条件相关且 LastUpdate∈近 24h | **16**（聚焦子集约 **12**） |

原始文件：`raw_pubmed.json` / `raw_crossref.json` / `raw_clinicaltrials.json`。

> 相对上一小时（UTC 02h：EDAT 91 / Crossref 63）索引量继续低位；Trials.gov 本窗更新条数略升（16/12 vs 11/9），多为 LastUpdate 行政刷新，不等于结果公布。
...
| PMID 42516535 日本 NCD | n=210 初次肝切除；风险计算器与长期预后关联 |
...
7. UTC 03h 相对 02h 的量级变化（EDAT −1、Crossref −6、CT 聚焦 +3）属预期昼夜波动，不代表领域“证据崩塌”。
