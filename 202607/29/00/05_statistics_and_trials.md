...
| 临床向标题过滤 | **58** |
| Crossref 去重（标题关键词过滤） | **51** |
| ClinicalTrials.gov 条件相关且 LastUpdate∈近 24h | **16**（聚焦子集约 **11**） |

原始文件：`raw_pubmed.json` / `raw_crossref.json` / `raw_clinicaltrials.json`。

> 相对前日晚间峰值（EDAT ~119–133 / Crossref ~100+），UTC 00h 为明显回落的安静窗。
...
| PMID 42508157 OPTN | 候名单 HCC n=8358；DDL 22.4%；MP 中心 DDL 19.7% vs SCS 26.2% |
| PMID 42512383 意大利登记 | 31 登记处 / 2805.7 万居民；2018 患病 53/10 万；HCC cure prevalence 23.3%；LEF 约 29 月（2015） |
...
| EMERALD-3（新闻层） | n=760；PFS HR 0.70；OS 未成熟；ORR 三联 38.9% / STRIDE+TACE 40.8% / TACE 27.0%（DCO2） |

---

## 3. ClinicalTrials.gov｜LastUpdate 2026-07-28–29（肝胆相关摘录）

完整 **16** 条见 `raw_clinicaltrials.json`。以下挑与 **HCC / 肝切除 / 胆道 / 肝移植路径** 更直接者：
...
| NCT06199232 / NCT05044819 / NCT07731373 等 | 见 JSON；结直肠肝转移、潜在肝损伤、PCOS 肝脂肪变性等 |
...
5. 本小时 EDAT 90 vs PDAT 18：说明大量“可见新增”是索引/录入事件，而非全部当日正式发表。  
6. 安静窗不等于“无新证据”——权威面板与会议层 III 期仍应交叉阅读。
